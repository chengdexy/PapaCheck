...
> 
> **⚠️ 实际部署注意（2026-07-14 验证）**：`--dir dist` 不可省略（编译产物在 `dist/index.js`，用 `--dir .` 入口找不到会持续 `FUNCTION_INVOCATION_FAILED`）；JWT 密钥走随包 `dist/jwt.secret` 文件而非 CLI 注入（`tcb config update fn` 不落盘）；SCF `/data` 只读，模块加载期禁止写文件。详见 CHEATSHEET.md「云函数部署」一节。
...
```bash
$env:DATABASE_URL="postgresql://papacheck:papacheck@localhost:5432/papacheck_test"
