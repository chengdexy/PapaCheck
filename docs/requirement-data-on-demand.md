...
> ⚠️ **状态注记（2026-07-19）**：本需求对应的前端实现（`Data` 层 + 按需端点切换）截至合并 `backup/pre-rewrite-2026-07-14` 入 `main` **尚未落地**。前端实际仍全量 `API.getData()` 拉取 `/api/data`；仅后端按需端点已上线（见 `docs/CHANGELOG.md`）。前端按需切换为待办重构。
