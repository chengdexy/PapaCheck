...
| 日期 | 变更 |
|------|------|
| 2026-06-17 | **代码审查修复 4 个 Issue**：`getBackupFilePath` 从硬编码改为读取配置（修复非默认备份目录下载失败）；`allStateKeys` 硬编码改为 `ALL_ALERT_KEYS` 共享常量；SMTP transport 添加 `close()` 修复连接泄漏；`pruneOldBackups` 循环内 `getOpsConfig()` 提升到循环外；全量 657 测试通过 |
