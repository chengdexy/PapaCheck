...
从 [Releases](https://github.com/chengdexy/PapaCheck/releases) 下载最新版 `PapaCheck-{version}.exe`（当前 v1.2.15），双击运行即可。
...
| **测试**          | Vitest（前端/服务端 362 测试）、pytest（Python 53 测试） |
...
```bash
# 全部测试
npm test                   # 前端 + 服务端测试（Vitest，362 个测试用例）
