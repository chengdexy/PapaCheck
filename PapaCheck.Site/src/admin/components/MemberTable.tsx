...
import { Copy, RefreshCw, Trash2, User, Baby } from 'lucide-react';
...
export default function MemberTable({ refreshKey }: { refreshKey: number }) {
  ...
  if (loadState === 'empty') {
    return (
      <div className="text-center py-10 text-[var(--color-ink-500)]">
        <p className="text-sm">暂无家庭成员</p>
        <p className="text-xs text-[var(--color-ink-400)] mt-1">使用下方表单添加第一个成员</p>
      </div>
    );
  }
  ...
  return (
    <div className="pc-table-wrap overflow-x-auto">
      <table className="pc-table">
        ...
        <tbody>
          {members.map(m => {
            const RoleIcon = m.role === 'parent' ? User : Baby;
            return (
              <tr key={m.id}>
                <td className="font-semibold text-[var(--color-ink-900)]">{m.nickname}</td>
                <td>
                  <span
                    className={`pc-badge ${
                      m.role === 'parent' ? 'pc-badge-parent' : 'pc-badge-child'
                    }`}
                  >
                    <RoleIcon size={12} />
                    {m.role === 'parent' ? '家长' : '孩子'}
                ...
                <td className="text-right">
                  {m.id !== userId ? (
                    ...
                    <span className="text-xs text-[var(--color-ink-400)]">（自己）</span>
                  )}
                </td>
              </tr>
            );
